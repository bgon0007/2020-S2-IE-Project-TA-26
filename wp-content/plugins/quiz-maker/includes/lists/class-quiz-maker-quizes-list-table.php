...
class Quizes_List_Table extends WP_List_Table{
    ...
    public function extra_tablenav($which) {        
        ...
        <a style="" href="?page=<?php echo $_REQUEST['page'] ?>" class="button"><?php echo __( "Clear filters", $this->plugin_name ); ?></a>
        <?php
    }
    ...
    public function add_or_edit_quizes($data){
        ...
        if( isset($data["quiz_action"]) && wp_verify_nonce( $data["quiz_action"],'quiz_action' ) ){
            ...
            $enable_questions_ordering_by_cat = (isset($data['ays_enable_questions_ordering_by_cat']) && $data['ays_enable_questions_ordering_by_cat'] == "on") ? 'on' : 'off';

            // Show schedule timer
            $show_schedule_timer = (isset($data['ays_quiz_show_timer']) && $data['ays_quiz_show_timer'] == 'on') ? 'on' : 'off';
            $show_timer_type = (isset($data['ays_show_timer_type']) && $data['ays_show_timer_type'] != '') ? $data['ays_show_timer_type'] : 'countdown';
